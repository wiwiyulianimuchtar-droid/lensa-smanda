...
export default function StudentLayout({ children }) {
  ...
  // If loading or profile doesn't exist/not SISWA, the AuthProvider handles redirects,
  // but let's show a loading screen just in case.
  if (loading) {
    return (
      <div style={{
        ...
        <div style={{fontSize: 18, fontWeight: 'bold'}}>Memuat Portal Siswa...</div>
      </div>
    );
  }
  ...
  if (profile && profile.role !== 'SISWA') {
    return (
      <div style={{
        ...
        <p style={{color: 'var(--text-muted)', marginBottom: 20}}>Portal ini khusus untuk Siswa SMAN 2 Bandung.</p>
