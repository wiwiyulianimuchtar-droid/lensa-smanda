...
// Polyfill window.crypto.subtle in insecure (HTTP) contexts
  ...
  if (!window.crypto.subtle) {
    ...
    window.crypto.subtle = {
      digest: async function(algorithm, data) {
        ...
        if (algoName !== 'SHA-256') {
          throw new Error('Unsupported algorithm in polyfill: ' + algoName);
        }
        ...
        if (data instanceof Uint8Array) {
          ...
          throw new Error('Unsupported data type in polyfill');
        }
        
        return sha256(buffer);
      }
    };
    console.warn("Supabase local IP polyfill: window.crypto.subtle defined successfully.");
  }
}
